...
tags: 'Google Lighthouse, webP, 지연 로딩'
date: '2023-03-25'
---
...
&emsp;블로그 gif 리소스들은 지연 로딩과 자동 리사이징 등을 지원하는 next/Image로 관리했어요. 다만 gif 파일의 크기가 큰 경우 Image 컴포넌트가 뷰포트에 들어왔을 때 서버에서 다운로드 받기 전까지는 placeholder나 공란으로 출력이 되는 상황이 반복됐습니다. 파일 크기와 네트워크 상태에 따라 상이하겠지만 1, 2초 내외로 기다려야 하는 경우도 있었어요.

<img src="/assets/markdown-image/React-블로그-성능-측정/서버-응답.png" alt="gif 서버 응답" width="800" height="600" />

<span>1.1 gif 서버 응답</span>

gif가 출력되는 블로그 포스트의 성능 섹션부터 확인해 보겠습니다.

<img src="/assets/markdown-image/React-블로그-성능-측정/performance.png" alt="lighthouse performance" width="550" height="300" />

<span>1.2 Performance</span>

lighthouse도 개선 사항으로 비디오 포맷의 사용을 권장하네요. WebM 포맷은 gif보다 훨씬 적은 용량으로 여러 브라우저 환경에서도 재생이 가능한 미디어 파일 포맷입니다. readme를 작성할 때 gif를 애용해서 사용해왔던지라 앱에서의 성능 고려는 해보지 않았었는데 media 포맷으로 변경 후 블로그 포스트에서는 gif 포맷의 사용 근거는 찾기 힘든 거 같습니다.
...
&emsp;본 블로그는 챗봇 시스템으로 채널톡을 사용하고 있어요. 성능 검사 결과 로컬 쿠키와 관련된 이슈가 출력이 됐는데 제품 팀에 문의해 보니 로컬 쿠키가 사용자 인증과 판별에만 사용되고 보안적으로는 문제가 되지 않는다는 답변을 받았어요. 출력된 이슈는 추측건대 쿠키가 자바스크립트로 조작이 가능해서 lighthouse에서 유의하고 인지하라는 정도로 생각돼요.
...
&emsp;블로그 앱의 성격상 많은 양의 데이터가 오가거나 서버로 복잡한 요청을 보내진 않지만 페이지 초기 렌더에 댓글 데이터에 대한 요청을 보내고 클라이언트 사이드에서 댓글 컴포넌트를 출력해요. 방문자가 언제나 글을 처음부터 끝까지 읽는 건 아니기 때문에 댓글 데이터는 페이지 최하단에 있는 댓글 컴포넌트가 뷰포트에 노출됐을 때만 Next 서버에 요청하고 관련 컴포넌트를 지연 로딩 처리하는 게 페이지 로드나 데이터 사용량 개선이 도움이 된다고 판단해서 방문자가 페이지 하단에 접근했을 경우에만 댓글 데이터 요청을 보내도록 했어요.

<video url="/assets/markdown-image/React-블로그-성능-측정/지연-로딩.webm"  width="100%" height="auto"><video />

<span>1.7 댓글 지연 로딩</span>
...
&emsp;NextJS 프레임워크가 지원하는 큰 장점 중 몇 가지가 서버 사이드 렌더링과 SEO 최적화에요. next/Head 컴포넌트, sitemap, 검색 콘솔 활용 등으로 검색 엔진이 인덱싱하고 크롤링 할 수 있게 신경을 썼는데 독자의 흥미를 돋우고 도움이 되는 양질의 글을 발행하는 것이 최선의 SEO 개선이라고 생각돼요.
